...
CompareModOperator(const bson_value_t *srcVal, const bson_value_t *modArrVal)
{
	 ...
	 * runtime rejected a document the index term (already normalized to
	 * int64) matched — same query, different answers depending on the plan.
	 * MongoDB truncates the value to a 64-bit integer without rounding. */
	bool checkFixedInteger = false;
